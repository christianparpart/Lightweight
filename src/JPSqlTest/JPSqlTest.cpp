...
std::ostream& operator<<(std::ostream& os, SqlResult<void> const& result)
{
    if (result)
...
std::ostream& operator<<(std::ostream& os, SqlDate const& date)
{
    auto const ymd = date.value;
    return os << std::format("SqlDate {{ {}-{}-{} }}", ymd.year(), ymd.month(), ymd.day());
}

std::ostream& operator<<(std::ostream& os, SqlTime const& time)
{
...
std::ostream& operator<<(std::ostream& os, SqlTimestamp const& timestamp)
{
    return os << std::format("SqlTimestamp {{ {} }}", timestamp.value);
}

template <typename T>
std::ostream& operator<<(std::ostream& os, SqlResult<T> const& result)
{
    if (result)
